...
def create_music_description(instruments, genre, time_signature, playtime, bars, pitch_range, key, tempo):
    ...
    # 생성되는 음악의 다양성을 위해 사용 가능한 아티스트 중 랜덤하게 추가
    ...
    with open(f"{project_path}/1-text2attribute_model/data/music_description.json", "w", encoding='utf-8') as f:
...
def run_model():
    ...
    try:
        # predict.sh 파일 실행
        process = subprocess.Popen(['bash', f'{project_path}/1-text2attribute_model/predict.sh'])
        ...
        # stage2_pre.py 파일 실행
        process = subprocess.Popen(['python3', f'{project_path}/1-text2attribute_model/stage2_pre.py'])
        current_pid = process.pid
        ...
        process = subprocess.Popen(['bash', f'{project_path}/2-attribute2music_model/interactive_1billion.sh', '0', '1', current_time])
        current_pid = process.pid
        ...
        # remi2midi.py 파일 실행
        process = subprocess.Popen(['python3', f'{project_path}/2-attribute2music_model/remi2midi.py', current_time])
        current_pid = process.pid
    ...
    file_urls = [
        f'http://{server_ip}/download_file/{current_time}.mid',
        f'http://{server_ip}/download_file/{current_time}.pdf'
    ]
...
def download_file(filename):
    ...
    # 파일 경로 설정
    file_path = os.path.join(f'{project_path}/2-attribute2music_model/generation/{current_time}/linear_mask-1billion-attribute2music/infer_test/topk15-t1.0-ngram0/0/midi', filename)
